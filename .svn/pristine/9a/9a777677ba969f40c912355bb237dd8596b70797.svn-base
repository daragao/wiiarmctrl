...
void Utils::TaitBryan2AxisAngle(double *angleArray, double roll, double pitch, double yaw){
	double rMatrix[3][3];
	getRotationMatrix(rMatrix,roll,pitch,yaw);
...
void Utils::getRotationMatrix(double rMatrix[3][3], double eulerAngles[3]){
	getRotationMatrix(rMatrix,eulerAngles[0],eulerAngles[1],eulerAngles[2]);
}

void Utils::getRotationMatrix(double rMatrix[3][3], double roll, double pitch, double yaw){
	double ch = cos(yaw);
    double sh = sin(yaw);
    double ca = cos(pitch);
    double sa = sin(pitch);
...
double Utils::lowPassFilter(double oldValue, double newValue){
	double result = 0;
	double weight = 0.05;
	result = (oldValue*(1-weight))+(newValue*weight);
	return result;
}

double Utils::getMatrixDeterminant( double matrix[3][3]){
	...
	h = matrix[2][1];
	i = matrix[2][2];
	double det = a*e*i + c*d*h - g*e*c - h*f*a -i*d*b;
	return det;
}
...
void Utils::makeRodriguesRotation(double newVector[3], double rotMatrix[3][3], double oldVector[3], double angle[3]){
	double newRotMatrix[3][3] = {{1,0,0},{0,1,0},{0,0,1}};
	getRodriguesRotMatrix(rotMatrix,angle[0],angle[1],angle[2],oldVector[0],oldVector[1],oldVector[2]);
	Utils::mult3DMatrix(rotMatrix,rotMatrix,newRotMatrix);
	Utils::rotateVector(newVector,rotMatrix,oldVector[0],oldVector[1],oldVector[2]);
}