...
void handle_event(struct wiimote_t* wm) {
	...
	if (wm->exp.type == EXP_NUNCHUK) {
		...
		//printf("dry = %u\n", mp->ry);
		//printf("drz   = %u\n", mp->rz);
		double drx = (double) mp->rx;
		double dry = (double) mp->ry;
		double drz = (double) mp->rz;
		
		printf("motion plus drx = %f\n", drx);
		printf("motion plus dry = %f\n", dry);
		printf("motion plus drz = %f\n", drz);
		//wiiuse_motion_sensing(wm, 1);
	}
}
...
void printEvent(struct wiimote_t* wm, byte* data, unsigned short len)
{
	switch (wm->event) {
		case WIIUSE_EVENT:
		...
		case WIIUSE_NUNCHUK_INSERTED:
			 ...
			 *	as the wiimote.
			 */
			//wiiuse_set_nunchuk_orient_threshold((struct nunchuk_t*)&wm->exp.nunchuk, 90.0f);
			//wiiuse_set_nunchuk_accel_threshold((struct nunchuk_t*)&wm->exp.nunchuk, 100);
...
int main(int argc, char** argv) {
	...
	/*
	char * adr = wiiuse_find_single(1);
	printf("found : %s\n", adr);
	
	if (!wiiuse_connect_single(wiimotes[0], adr))
		return 0;
	 */
	...
	wiiuse_set_leds(wiimotes[4], WIIMOTE_LED_1|WIIMOTE_LED_2);
	wiiuse_set_leds(wiimotes[5], WIIMOTE_LED_1|WIIMOTE_LED_3);
...
//	wiiuse_rumble(wiimotes[1], 0);
	...
	//wiiuse_poll(wiimotes, MAX_WIIMOTES, printEvent);

	/* wiiuse_start_event_loop(printEvent);
