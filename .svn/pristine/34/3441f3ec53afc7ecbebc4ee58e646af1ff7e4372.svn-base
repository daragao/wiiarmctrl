...
bool PortCtrlMod::updateModule(){
	Vector *handJoints = _christianPort->read(false);
	Vector newHandJoints(9);
	if(handJoints!=NULL){
		for(int i = 7; i <= 15; i++)
			newHandJoints[i-7] = (*handJoints)[i];
	}
	for(int i = 0; i != _niCubgen; i++){
		_icubGen[i]->setChristianHandPos(newHandJoints);
...
bool PortCtrlMod::configure(ResourceFinder &rf){
	...
	if(_useSkeleton){
		if(partName=="all"){
			...
			//_icubGen[3] = new iCubMotorCtrl(localPortPrefix,robotName,"left_leg");
			//((iCubMotorCtrl*)_icubGen[3])->setSpeed(acc,speed);
			//_icubGen[4] = new iCubMotorCtrl(localPortPrefix,robotName,"right_leg");
			//((iCubMotorCtrl*)_icubGen[4])->setSpeed(acc,speed);
	...
	string christianPortName = "/christianPort";
	_christianPort = new BufferedPort<Vector>();
	if(!_christianPort->open(christianPortName.c_str())){
		printf("Christian failed!\n");
		return false;
	}
	//_christianPort->useCallback(*this);
